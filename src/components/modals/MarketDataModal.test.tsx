...
import strategiesReducer, { setMarketDataSubscriptions, setMarketDataSubscriptionsStatus } from '../../store/slices/strategiesSlice';
...
let directoryResponseSymbols: string[] = [];
let directoryResponseIntervals: string[] = [];
let directoryPreferredSymbol: string | null = null;
...
const initialSymbolRangeResponse: RangePayload = {
  entries: [
    {
      symbol: 'ES',
      data_type: 'bar_1m',
...
const refreshedSymbolRangeResponse: RangePayload = {
  entries: [
    {
      symbol: 'ES',
      data_type: 'bar_1m',
...
let symbolRangeResponse: RangePayload = initialSymbolRangeResponse;
...
const baseStrategiesState = strategiesReducer(undefined, { type: '@@INIT' } as never);

const createStore = (
  overrides?: {
    strategies?: Partial<typeof baseStrategiesState>;
    auth?: Record<string, unknown>;
    market?: Record<string, unknown>;
  }
) => {
  const authState = (overrides?.auth ?? { token: 'test-token' }) as Record<string, unknown>;
  const marketState =
    (overrides?.market ?? {
      symbols: [
        {
...
const clickCoverageTab = async (user: ReturnType<typeof userEvent.setup>) => {
  const coverageTab = (await waitFor(() => {
    const button = Array.from(document.querySelectorAll('button')).find((element) =>
      element.textContent?.includes('覆盖统计')
    );
    if (!button) {
...
await (async () => {
  ...
  const renderResult = render(
    <Provider store={store}>
      <MarketDataModal open onClose={() => undefined} />
    </Provider>
  );
  const user = userEvent.setup({ document: window.document });
  await clickCoverageTab(user);
  await waitFor(() => {
    const refreshValue = document.querySelector('.refreshValue') as HTMLElement | null;
  ...
  await waitFor(() => {
    const trigger = Array.from(document.querySelectorAll('button')).find((element) =>
      element.textContent?.includes('Backfill')
  ...
  const backfillButton = Array.from(document.querySelectorAll('button')).find((element) =>
    element.textContent?.includes('Backfill')
  ) as HTMLButtonElement | undefined;
  ...
  await waitFor(() => {
    ...
    const fallbacks = parsed.ib_client_id_fallbacks as number[] | undefined;
...
await (async () => {
  ...
  const subscriptionsTab = (await waitFor(() => {
    const button = Array.from(document.querySelectorAll('button')).find((element) =>
      element.textContent?.includes('实时订阅')
    );
    if (!button) {
  ...
  const sampleSubscriptions: ActiveSubscriptionSummaryPayload[] = [
    {
  ...
  await waitFor(() => {
    ...
    const strategyAlphaRow = Array.from(rows).find(row => 
      row.textContent?.includes('Strategy Alpha')
    );
    if (!strategyAlphaRow) {
    ...
    const strategyBetaRow = Array.from(rows).find(row => row.textContent?.includes('Strategy Beta'));
    if (!strategyBetaRow) {
      throw new Error('Ticker subscriber row should be rendered for Strategy Beta');
    }
    if (!strategyBetaRow.textContent?.includes('10 小时前')) {
      throw new Error('Ticker subscriber subscribedAt timestamp should render as relative hours');
    }
    if (!strategyBetaRow.textContent?.includes('40 分钟前')) {
    ...
    const stopButton = Array.from(document.querySelectorAll('button')).find((element) =>
      element.textContent === '取消订阅'
    );
    if (!stopButton) {
...
await (async () => {
  ...
  const sharedOwnerSubscriptions: ActiveSubscriptionSummaryPayload[] = [
    {
...
await (async () => {
  ...
  const verboseStatus =
    'Stream update: the upstream provider reported a transient spike in load that should recover shortly after automated retries are processed.';
  ...
  const user = userEvent.setup({ document: window.document });
  const subscriptionsTab = Array.from(document.querySelectorAll('button')).find((element) =>
    element.textContent === '实时订阅'
  );
  if (subscriptionsTab) {
    await user.click(subscriptionsTab);
  }
...
await (async () => {
  ...
  await waitFor(() => {
    const select = document.getElementById('market-data-symbol') as HTMLSelectElement | null;
    if (!select) {
  ...
  const refreshButton = Array.from(document.querySelectorAll('button')).find((element) =>
    element.textContent?.includes('刷新目录')
  ) as HTMLButtonElement | undefined;
  if (!refreshButton) {
...
await (async () => {
  ...
  const select = (await waitFor(() => {
    const element = document.getElementById('market-data-symbol') as HTMLSelectElement | null;
    if (!element) {
      throw new Error('Symbol selector should be rendered');
    }
    return element;
  ...
  const initialDirectoryCalls = fetchCalls.filter((call) =>
  ...
  await waitFor(() => {
    ...
    const directoryCalls = fetchCalls.filter((call) =>
      call.url.includes('/data/market/catalog')
    ).length;
  ...
  await waitFor(() => {
    const symbolSelect = document.getElementById('market-data-symbol') as HTMLSelectElement | null;
    if (!symbolSelect) {
...
// Resubscribe action and Bars display test
await (async () => {
  fetchCalls.length = 0;
  const store = createStore();
  const renderResult = render(
      ...
      streams: [
        {
          ...
          ownerCount: 1,
          totalReferences: 1,
          requestId: 'req-bars-2',
          subscribers: [
            {
              ownerId: 'strategy-service',
              referenceCount: 1,
              source: 'Momentum5',
              name: 'Momentum5',
              subscribedAt: '2023-01-02T08:00:00Z',
  ...
  await waitFor(() => {
    const rows = document.querySelectorAll('[data-testid="market-data-subs-row"]');
    if (rows.length !== 1) {
      throw new Error('Bars subscriber row should render');
    }
    const text = rows[0]?.textContent ?? '';
    if (!text.includes('Momentum5')) {
      throw new Error('Bars subscriber should display Momentum5 as name/source');
    }
  });

  (globalThis as unknown as Record<string, unknown>).fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    const url = typeof input === 'string' ? input : input instanceof URL ? input.toString() : '';
    fetchCalls.push({ url, init });
    if (url.includes('/market/subscription/resubscribe')) {
      return createJsonResponse({ active: 1, restarted: 1, status: 'complete' });
    }
    return createJsonResponse({});
  };

  const resubscribeButton = (await waitFor(() => {
    const btn = Array.from(document.querySelectorAll('button')).find((el) => el.textContent === '重新订阅');
    if (!btn) {
      throw new Error('Resubscribe button should be available');
    }
    return btn;
  })) as HTMLButtonElement;
  ...
  await waitFor(() => {
    const call = fetchCalls.find((c) => c.url.includes('/market/subscription/resubscribe'));
    if (!call) {
      throw new Error('Resubscribe request should be issued');
    }
    if ((call.init?.method ?? 'GET').toUpperCase() !== 'POST') {
      throw new Error('Resubscribe request should use POST method');
    }
  });
...
console.log('MarketDataModal resubscribe and Bars display test passed');
